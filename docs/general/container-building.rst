...
`docker <https://www.docker.com>`_ is required. The development team prefers ``podman``, but ``docker`` (or other
compatible container CLI) may generally be substituted without consequence.
